...
메소드 내에서 정의하는 **기본 자료형(primitive type) 에 해당하는 지역변수, 매개변수 , 리턴값이 저장된다.**
지역변수란 메소드 내에서 정의된 변수를 말한다. 뿐만 아니라 **Heap 영역에 생성된 객체들을 참조하는 참조변수가 저장된다.**

Stack 자료구조는 **Last in First Out** 구조로 마지막에 들어온 값이 가장 먼저 나간다. 즉  메소드 호출시 스택 프레임이 생성되고  메소드 안에 정의된 기본자료형인 매개변수, 지역변수, 리턴값을 저장한다. 그리고 메소드가 호출 범위가 종료되면 스택에서 제거된다.
...
클래스에 대한 정보가 이 영역에 존재하기 때문에 인스턴스 생성하기 위해서는 이 Method Area를 참조해야한다.또한 모든 쓰레드가 이 영역을 공유한다.  JVM이 실행될때 Method area는 생성되며, (java 명령어로 실행할때 main 메소드 호출하며 실행)  클래스가 **사용되면** JVM은 해당 클래스의 `.class` 파일을 읽어 분석해 클래스의 인스턴스 변수, 메소드 정보 등을 method area에  적재한다. 이 때 클래스 변수(static 변수) 도 이 영역에 함께 생성된다.

참고로 클래스가 로드 되는 시점은 해당 클래스가 사용되기 위해 호출되는 때이다.  프로그램이 실행되었다고 모든 코드가 저장되어 있는 상태가 아니다. )
